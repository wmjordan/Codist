...
sealed class CSharpMargin : MarginElementBase, IWpfTextViewMargin
{
	...
	sealed class MemberMarker
	{
		...
		static void ParseSyntaxNode(ITextSnapshot snapshot, SyntaxNode parentSyntaxNode, CodeBlock parentCodeBlockNode, int level, CancellationToken token) {
			if (token.IsCancellationRequested
				|| parentSyntaxNode.SyntaxTree.Length > snapshot.Length) {
		...
		static CodeMemberType MatchDeclaration(SyntaxNode node) {
			return node.Kind() switch {
				SyntaxKind.ClassDeclaration
					or CodeAnalysisHelper.RecordDeclaration => CodeMemberType.Class,
				SyntaxKind.InterfaceDeclaration => CodeMemberType.Interface,
				SyntaxKind.StructDeclaration => CodeMemberType.Struct,
				SyntaxKind.EnumDeclaration => CodeMemberType.Enum,
				SyntaxKind.ConstructorDeclaration
					or SyntaxKind.DestructorDeclaration => CodeMemberType.Constructor,
				SyntaxKind.MethodDeclaration
					or SyntaxKind.OperatorDeclaration
					or SyntaxKind.ConversionOperatorDeclaration => CodeMemberType.Method,
				SyntaxKind.IndexerDeclaration
					or SyntaxKind.PropertyDeclaration => CodeMemberType.Property,
				SyntaxKind.FieldDeclaration => CodeMemberType.Field,
				SyntaxKind.EventDeclaration
					or SyntaxKind.EventFieldDeclaration => CodeMemberType.Event,
				SyntaxKind.DelegateDeclaration => CodeMemberType.Delegate,
				_ => CodeMemberType.Unknown,
			};
		}
		...
		sealed class CodeBlock
		{
			...
			public IList<CodeBlock> Children { get; } = [];
		...
		sealed class PenStore
		{
			...
			internal Pen GetPenForCodeMemberType(CodeMemberType memberType) {
				return memberType switch {
					CodeMemberType.Class => Class,
					CodeMemberType.Interface => Interface,
					CodeMemberType.Struct => Struct,
					CodeMemberType.Enum => Enum,
					CodeMemberType.Event => Event,
					CodeMemberType.Delegate => Delegate,
					CodeMemberType.Constructor => Constructor,
					CodeMemberType.Property => Property,
					CodeMemberType.Method => Method,
					CodeMemberType.Field => Field,
					_ => null,
				};
			}
		}

		sealed class DrawText(FormattedText text, double ySpan, Point point)
		{
			public readonly FormattedText Text = text;
			public readonly Point Point = point;
			public readonly double YSpan = ySpan;
	...
	sealed class SymbolReferenceMarker(IVerticalScrollBar verticalScrollbar, CSharpMargin margin)
	{
		const double MarkerMargin = 1;
		static PenStore __PenStore;
		IVerticalScrollBar _ScrollBar = verticalScrollbar;
		CSharpMargin _Margin = margin;
		...
		readonly struct ReferenceItem(int position, SymbolUsageKind usage)
		{
			public readonly int Position = position;
			public readonly SymbolUsageKind Usage = usage;
		}
	}
}
