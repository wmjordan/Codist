		...
		sealed class CSharpCompletionTooltip : ContentControl
		{
			...
			static StackPanel Render(ImmutableArray<ISymbol> symbols, SemanticModel semanticModel, StackPanel content) {
				...
				if (symbols.Length > 1) {
					...
					op.Add(new TextBlock().Append(ThemeHelper.GetImage(IconIds.MethodOverloads).WrapMargin(WpfHelper.GlyphMargin)).Append(R.T_MethodOverload, true));
					for (int i = 1; i < symbols.Length; i++) {
						ISymbol s = symbols[i];
						if (s is IMethodSymbol m) {
							op.Add(SymbolFormatter.Instance.ShowParameters(new TextBlock().Append(ThemeHelper.GetImage(m.GetImageId()).WrapMargin(WpfHelper.GlyphMargin)).AddSymbol(m, false, SymbolFormatter.SemiTransparent), m.Parameters, true, true));
						}
					}
					content.Add(op);
				}
