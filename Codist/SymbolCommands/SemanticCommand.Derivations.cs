	...
	: HierarchicalListCommand<INamedTypeSymbol>
{
	...
	public override async Task<(TypeRelations, IReadOnlyList<INamedTypeSymbol>)> PrepareListDataAsync(CancellationToken cancellationToken) {
		...
		if (MatchNamespace) {
			classes = classes.Where(new SymbolNamespaceFilter(Symbol).Filter);
		}
		if (DirectDerive) {
...
{
	static readonly OptionDescriptor[] __Options = [new OptionDescriptor(IconIds.DirectDerive, CommandOptions.DirectDerive, R.CMDT_FindDirectImplementations), OD.CurrentProjectScope, OD.RelatedProjectsScope, OD.SourceCodeScope, OD.ExternalScope];
	...
	public override async Task<ImmutableArray<ISymbol>> PrepareListDataAsync(CancellationToken cancellationToken) {
		...
		return await Symbol.FindImplementationsAsync(Context.Document.Project.Solution, directImplementation, source, ns, projects, cancellationToken).ConfigureAwait(false);
	}
...
{
	static readonly OptionDescriptor[] __Options = [OD.DirectDerive, OD.SourceCodeScope, OD.ExternalScope, OD.NamespaceScope];
	string _ResultLabel;
	...
	public override async Task<ImmutableArray<INamedTypeSymbol>> PrepareListDataAsync(CancellationToken cancellationToken) {
		...
		return await (Symbol as INamedTypeSymbol).FindDerivedInterfacesAsync(Context.Document.Project, directDerive, source, ns, cancellationToken).ConfigureAwait(false);
	}
	...
	: AnalysisListCommandBase<ImmutableArray<ISymbol>>
{
	...
	public override async Task<ImmutableArray<ISymbol>> PrepareListDataAsync(CancellationToken cancellationToken) {
		...
		var source = MakeSourceFilterFromOption(Options);
		var ns = MakeNamespaceFilterFromOption(Options);
		return (await Symbol.FindOverridesAsync(Context.Document.Project.Solution, source, ns, projects, cancellationToken).ConfigureAwait(false)).MakeSortedSymbolArray();
	}
