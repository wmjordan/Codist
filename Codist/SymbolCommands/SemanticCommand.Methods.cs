	...
	: CommonListCommand<ISymbol>
{
	...
	public override Task<ImmutableArray<ISymbol>> PrepareListDataAsync(CancellationToken cancellationToken) {
		...
		return Task.Run(() => (Context.SemanticModel.Compilation.FindMethodBySignature(Symbol, excludeGenerics, source, ns, cancellationToken)).MakeSortedSymbolArray());
	}
	...
	: CommonListCommand<IMethodSymbol>
{
	...
	public override async Task<ImmutableArray<IMethodSymbol>> PrepareListDataAsync(CancellationToken cancellationToken) {
		var source = MakeSourceFilterFromOption(Options);
		var ns = MakeNamespaceFilterFromOption(Options);
		return await (Symbol as ITypeSymbol).FindExtensionMethodsAsync(Context.Document.Project, StrictMatch, MatchTypeArgument, source, ns, cancellationToken).ConfigureAwait(false);
	}
	...
	: CommonListCommand<IMethodSymbol>
{
	...
	public override async Task<ImmutableArray<IMethodSymbol>> PrepareListDataAsync(CancellationToken cancellationToken) {
		var source = MakeSourceFilterFromOption(Options);
		var ns = MakeNamespaceFilterFromOption(Options);
		return await Symbol.GetReturnType().FindExtensionMethodsAsync(Context.Document.Project, StrictMatch, MatchTypeArgument, source, ns, cancellationToken).ConfigureAwait(false);
	}
