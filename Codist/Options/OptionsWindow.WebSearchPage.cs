...
sealed partial class OptionsWindow
{
	sealed class WebSearchPage : OptionPageFactory
	{
		...
		sealed class PageControl : OptionPage
		{
			...
			readonly Button _BrowseBrowserPath, _AddButton, _RemoveButton, _MoveUpButton, _ResetButton;
			...
			public PageControl() {
				...
				_Url.GotFocus += HandleTextGotFocus;
			...
			void RefreshSearchEngineUI() {
				...
				if (_RemoveButton.IsEnabled = _Name.IsEnabled = _Url.IsEnabled = se != null) {
			...
			void HandleTextGotFocus(object sender, RoutedEventArgs e) {
				var textBox = (TextBox)sender;
				textBox.LostFocus += UpdateSearchEngine;
			}

			void UpdateSearchEngine(object sender, RoutedEventArgs e) {
				var textBox = (TextBox)sender;
				textBox.LostFocus -= UpdateSearchEngine;
				...
				bool changed = false;
				if (textBox == _Name && _Name.Text != se.Name) {
					se.Name = _Name.Text;
					changed = true;
				}
				else if (textBox == _Url && _Url.Text != se.Pattern) {
