...
sealed class OptionToggleButton : ThemedImageButton
{
	readonly Action<bool?> _CheckEventHandler;
	bool? _State;

	public OptionToggleButton(int imageId, string toolTip) : base(imageId, String.Empty) {
		ToolTip = new CommandToolTip(imageId, toolTip);
	}
	public OptionToggleButton(int imageId, string toolTip, Action<bool?> checkEventHandler) : this(imageId, toolTip) {
		_CheckEventHandler = checkEventHandler;
	}
	public OptionToggleButton(int imageId, string toolTip, bool? initialValue, Action<bool?> checkEventHandler) : this(imageId, toolTip) {
		_State = initialValue;
		SetStateIcon(initialValue);
		_CheckEventHandler = checkEventHandler;
	}

	public bool? State {
		get => _State;
		set => SetStateIcon(_State = value);
	}

	void SetStateIcon(bool? initialValue) {
		Header.Inlines.Clear();
		var items = (Content as StackPanel).Children;
		items.RemoveAndDisposeAt(1);
		items.Add(VsImageHelper.GetImage(!initialValue.HasValue ? IconIds.Default : initialValue == true ? IconIds.Enabled : IconIds.Disabled));
	}

	protected override void OnClick() {
		base.OnClick();
		SetStateIcon(_State = !_State.HasValue
			? true
			: _State == true
				? false
				: null);
		_CheckEventHandler?.Invoke(_State);
	}
}
